...
Les réseaux de Petri, de manière générale, évoquent une sémantique discrète sur un graphe biparti entre un ensemble de places et de transitions, reliés par des arcs lestés. Considérons le réseau de la figure \ref{fig:petri-net-example}.
\begin{figure}[h]
    \centering
...
\end{figure}
Nous identifions : \begin{itemize}
...
Il apparaît clairement que l'on peut tirer indéfiniment la transition $t_1$ puisqu'elle ne dispose d'aucun arc entrant, donc aucune précondition. Ainsi, $p_1$ peut contenir un nombre arbitrairement grand de jetons et, par conséquent, le graphe est de taille infinie.
...
Cette définition permet, indépendamment de l'ordre, de savoir si l'on peut construire une séquence de tirs $\sigma$ parmi les transitions de $[\![v]\!]$ et ainsi accéder au marquage $m$. Supposons maintenant que $\mathcal{N}$ est un CEPN par un unique arc de test à 0. Cela implique qu'il existe une place $p_0 \in P$ et une transition $t_0 \in T$ tels que $\inhibitor(p_0, t_0) = 1$. Par conséquent, nous remplaçons les ensembles $\preb{[\![v]\!]}, \postb{[\![v]\!]}$ respectivement par $\preb{[\![v]\!]} \setminus \{p_0\}, \postb{[\![v]\!]} \setminus \{p_0\}$.
